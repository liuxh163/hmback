...
// 获取所有附加项列表
router.get('/v1/attendants/{?target=01,pages=1,pageNum=10}', tokenMw, async (ctx, next) => {
    await attendantController.index(ctx)
})
// 新建产品附加项
router.post('/v1/attendants', tokenMw, async (ctx, next) => {
    await attendantController.create(ctx)
})
// 更新产品附加项
router.put('/v1/attendants/:id', tokenMw, async (ctx, next) => {
    await attendantController.update(ctx)
})
// 停用产品附加项
router.put('/v1/attendants/:id/halt', tokenMw, async (ctx, next) => {
    await attendantController.halt(ctx)
})
// 启用产品附加项
router.put('/v1/attendants/:id/awaken', tokenMw, async (ctx, next) => {
